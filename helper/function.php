...
function getAllNumberKategori($namakategori)
{
    global $koneksi;
    $q = mysqli_query($koneksi, "SELECT * FROM `nomor` where `nama_kategori`='$namakategori'");
    $arr = [];
...
function url_wa()
{
    return getSingleValDB("numbers", "user_id", "1", "url_gateway");
}

function sender()
{
    return getSingleValDB("numbers", "user_id", "1", "body");
}
...
function api_key()
{
    return getSingleValDB("users", "id", "1", "api_key");
}
...
function sendMSGbtn($number, $msg, $footer, $btn1, $btn2)
{
...
function sendMedia($number, $msg, $file)
{
